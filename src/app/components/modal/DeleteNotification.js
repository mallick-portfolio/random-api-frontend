import { setShowDeleteNotificationModal } from "@/app/store/reducer/modalSlice";
...
import { useDeleteNotificationMutation } from "@/app/store/api/notificationApi";

const DeleteNotification = () => {
  const dispatch = useDispatch();

  const { showDeleteNotificationModal, selectedNotificationId } = useSelector(
    (state) => state.modal
  );
  ...
  // api call
  const [handleDelete, { data, isLoading }] = useDeleteNotificationMutation();

  useEffect(() => {
    if (data && data.success) {
      toast.success(data.message);
      dispatch(setShowDeleteNotificationModal(false));
                ...
                <div className="relative p-6 flex-auto">
                  <div className="mb-2">
                    <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                      Are you want to sure delete this notification
                    </label>
                  </div>
                </div>
                {/*footer*/}
